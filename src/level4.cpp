...
Level4::Level4(int width, int height, const char* effect_fragment_shader):
    width(width),
    height(height),
    start(false),
    collision(false),
    ...
    effectShader2(width,height,"src/shaders/vertex.glsl","src/shaders/sin_fragment.glsl"),
    effectShader3(width,height,"src/shaders/vertex.glsl","src/shaders/rainbow_fragment.glsl")
{
    generatePipes(5);
    for(int i=0; i<2; i++){
...
void Level4::processInput(float dt, int keys[])
{
...
void Level4::render()
{
    ...
    for(Sprite& background : backgrounds) background.draw();
    player.draw();
    for(auto& pipe : pipes) { pipe.first.draw(); pipe.second.draw(); }
    ...
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    effectShader3.use();
    effectShader3.setFloat("time", player.x);
    buffer2.draw();

    effectShader->use(); 
    effectShader->setFloat("playerX", (player.x + 40 - shader->cameraX) / width);
    effectShader->setFloat("playerY", 1.0f - (player.y + 40) / height);
    ...
    for(int i = 0; i < 4; i++) {
        if(ping) {
            ...
            glBindTexture(GL_TEXTURE_2D, buffer1.texture);
            buffer1.draw();
        } else {
            buffer1.bind();
            glClear(GL_COLOR_BUFFER_BIT);
            glBindTexture(GL_TEXTURE_2D, buffer2.texture);
            buffer2.draw();
        }
        ping = !ping;
    }
...
void Level4::generatePipes(int n)
{
    srand(time(0));
    pipe_position=1200;
    float gap=350;
